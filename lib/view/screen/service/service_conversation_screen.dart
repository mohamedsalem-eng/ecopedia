...
import 'package:mechatronia_app/view/basewidget/custom_loader.dart';
import 'package:provider/provider.dart';

class ServiceConversationScreen extends StatelessWidget {
  final ServiceTicketModel serviceTicketModel;
  ServiceConversationScreen({@required this.serviceTicketModel});
  ...
  Widget build(BuildContext context) {
    if (Provider.of<AuthProvider>(context, listen: false).isLoggedIn()) {
      Provider.of<ServiceProvider>(context, listen: false).getserviceReplyList(context, serviceTicketModel.id);
      Provider.of<ServiceProvider>(context, listen: false).getCount(context);
    }

    return CustomExpandedAppBar(
      title: getTranslated('service_enquiry_conversation', context),
      isGuestCheck: true,
      child: Column(children: [
        ...
        //     Text(
        //       'Place date: ${DateConverter.localDateToIsoStringAMPM(DateTime.parse(serviceTicketModel.createdAt))}',
        //       style: titilliumRegular.copyWith(fontSize: Dimensions.FONT_SIZE_SMALL),
        //     ),
        //     Text(serviceTicketModel.subject, style: titilliumSemiBold),
        //     Row(children: [
        //       Icon(Icons.notifications, color: ColorResources.getPrimary(context), size: 20),
        //       SizedBox(width: Dimensions.PADDING_SIZE_SMALL),
        //       Expanded(child: Text(serviceTicketModel.type, style: titilliumSemiBold)),
        //       TextButton(
        //         onPressed: null,
        //         style: TextButton.styleFrom(
        //           backgroundColor: serviceTicketModel.status == 1 ? ColorResources.getGreen(context) : Theme.of(context).primaryColor,
        //         ),
        //         child: Text(
        //           serviceTicketModel.status == 0 ? getTranslated('pending', context) : getTranslated('solved', context),
        ...
        Expanded(child: Consumer<ServiceProvider>(builder: (context, support, child) {
          return support.serviceReplyList != null
              ? ListView.builder(
                  itemCount: support.serviceReplyList.length,
                  reverse: true,
                  itemBuilder: (context, index) {
                    bool _isMe = support.serviceReplyList[index].customerMessage != null;
                    String _message = _isMe
                        ? support.serviceReplyList[index].customerMessage
                        : support.serviceReplyList[index].adminMessage;
                    String dateTime = DateConverter.localDateToIsoStringAMPM(
                        DateTime.parse(support.serviceReplyList[index].createdAt));
                    ...
                    return Container(
                      ...
                      child: Column(
                          ...
                          children: [
                            ...
                            _message != null
                                ? Text(_message, style: titilliumRegular.copyWith(fontSize: Dimensions.FONT_SIZE_SMALL))
                                : SizedBox.shrink(),
                  ...
                  onTap: () {
                    if (_controller.text.isNotEmpty) {
                      Provider.of<ServiceProvider>(context, listen: false)
                          .sendReply(context, serviceTicketModel.id, _controller.text);
                      _controller.text = '';
                    } else {}
                  },
                  child: Icon(
