import 'package:mechatronia_app/data/model/response/support_reply_model.dart';
import 'package:mechatronia_app/helper/api_checker.dart';
import 'package:mechatronia_app/helper/date_converter.dart';
import 'package:flutter/material.dart';
import 'package:mechatronia_app/data/model/body/support_ticket_body.dart';
...
import 'package:mechatronia_app/data/model/response/support_ticket_model.dart';
...
class SupportTicketProvider extends ChangeNotifier {
  ...
  List<SupportReplyModel> get supportReplyList =>
      _supportReplyList != null ? _supportReplyList.reversed.toList() : _supportReplyList;
  bool get isLoading => _isLoading;

  void sendSupportTicket(SupportTicketBody supportTicketBody, Function(bool isSuccess, String message) callback,
  ...
  Future<void> getSupportTicketList(BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      apiResponse.response.data
          .forEach((supportTicket) => _supportTicketList.add(SupportTicketModel.fromJson(supportTicket)));
    } else {
  ...
  Future<void> getSupportTicketReplyList(BuildContext context, int ticketID) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      _supportReplyList = [];
      apiResponse.response.data
          .forEach((supportReply) => _supportReplyList.add(SupportReplyModel.fromJson(supportReply)));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
  ...
  Future<void> sendReply(BuildContext context, int ticketID, String message) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      _supportReplyList.add(
          SupportReplyModel(customerMessage: message, createdAt: DateConverter.localDateToIsoString(DateTime.now())));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
