...
import 'package:mechatronia_app/data/model/response/chosen_shipping_method.dart';
import 'package:mechatronia_app/data/model/response/product_model.dart';
import 'package:mechatronia_app/data/model/response/response_model.dart';
import 'package:mechatronia_app/data/model/response/shipping_method_model.dart';
import 'package:mechatronia_app/data/model/response/shipping_model.dart';
...
class CartProvider extends ChangeNotifier {
  ...
  Future<void> addToCartAPI(CartModel cart, Function callback, BuildContext context, List<ChoiceOptions> choices,
    ...
    for (int i = 0; i < sellerIdList.length; i++) {
      ...
      if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
        ...
        apiResponse.response.data
            .forEach((shipping) => _shippingMethodList.add(ShippingMethodModel.fromJson(shipping)));
  ...
  Future<void> getChosenShippingMethod(BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      _chosenShippingList = [];
      apiResponse.response.data
          .forEach((shipping) => _chosenShippingList.add(ChosenShippingMethodModel.fromJson(shipping)));
      notifyListeners();
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
  ...
  void setShippingCost(cost) {
    _shippingCost = double.tryParse(cost.toString()) ?? 0.0;
    notifyListeners();
  }

  Future addShippingMethod(BuildContext context, int id, String cartGroupId, String city, Function callback) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await cartRepo.addShippingMethod(id, cartGroupId, city);
    _isLoading = false;
    print("CALLLEDD!!!!");
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      setShippingCost(apiResponse.response?.data['cost'] ?? "0");
      getChosenShippingMethod(context);
      //String message = map["message"];
      callback(true, '');
      notifyListeners();
