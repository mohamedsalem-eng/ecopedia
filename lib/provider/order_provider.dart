...
import 'package:mechatronia_app/data/model/response/refund_info_model.dart';
import 'package:mechatronia_app/data/model/response/refund_result_model.dart';
...
class OrderProvider with ChangeNotifier {
  ...
  int get billingAddressIndex => _addressIndex;
  int _shippingIndex;
  ...
  Future<void> placeOrder(OrderPlaceModel orderPlaceModel, Function callback, List<CartModel> cartList,
      String addressID, String couponCode, String billingAddressId, String orderNote) async {
    _isLoading = true;
    notifyListeners();
  ...
  Future<void> initShippingList(BuildContext context, int sellerID) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      apiResponse.response.data
          .forEach((shippingMethod) => _shippingList.add(ShippingMethodModel.fromJson(shippingMethod)));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
  ...
  Future<http.StreamedResponse> refundRequest(
      BuildContext context, int orderDetailsId, double amount, String refundReason, String token) async {
    _isLoading = true;
    notifyListeners();
    http.StreamedResponse response =
        await orderRepo.refundRequest(orderDetailsId, amount, refundReason, refundImage, token);
