...
class CartRepo {
  ...
  Future<ApiResponse> addToCartListData(
      CartModel cart, List<ChoiceOptions> choiceOptions, List<int> variationIndexes) async {
  ...
  Future<ApiResponse> updateQuantity(int key, int quantity) async {
    ...
    try {
      final response = await dioClient
          .post(AppConstants.UPDATE_CART_QUANTITY_URI, data: {'_method': 'put', 'key': key, 'quantity': quantity});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> removeFromCart(int key) async {
    try {
      final response = await dioClient.post(AppConstants.REMOVE_FROM_CART_URI, data: {'_method': 'delete', 'key': key});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> addShippingMethod(int id, String cartGroupId, String city) async {
    print('===>${{"id": id, "cart_group_id": cartGroupId, "city": city}}');
    try {
      final response = await dioClient
          .post(AppConstants.CHOOSE_SHIPPING_METHOD, data: {"id": id, "cart_group_id": cartGroupId, "city": city});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
