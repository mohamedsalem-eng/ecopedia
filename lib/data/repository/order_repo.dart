...
class OrderRepo {
  ...
  Future<ApiResponse> getShippingMethod(int sellerId) async {
    try {
      final response = sellerId == 1
          ? await dioClient.get('${AppConstants.GET_SHIPPING_METHOD}/$sellerId/admin')
  ...
  Future<http.StreamedResponse> refundRequest(
      int orderDetailsId, double amount, String refundReason, List<XFile> file, String token) async {
    http.MultipartRequest request =
        http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.REFUND_REQUEST_URI}'));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    for (int i = 0; i < file.length; i++) {
      if (file != null) {
        print('bangladesh======');
        Uint8List _list = await file[i].readAsBytes();
        var part = http.MultipartFile('images[]', file[i].readAsBytes().asStream(), _list.length,
  ...
  Future<ApiResponse> getRefundInfo(int orderDetailsId) async {
    try {
      final response =
          await dioClient.get('${AppConstants.REFUND_REQUEST_PRE_REQ_URI}?order_details_id=$orderDetailsId');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
